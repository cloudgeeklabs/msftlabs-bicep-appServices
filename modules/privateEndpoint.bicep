// ============ //
// Parameters   //
// ============ //

@description('Required. The name of the App Service for the private endpoint.')
param appServiceName string

@description('Required. The resource ID of the App Service.')
param appServiceResourceId string

@description('Required. The resource ID of the subnet for the private endpoint.')
param subnetResourceId string

@description('Optional. Azure region for deployment.')
param location string = resourceGroup().location

@description('Optional. The name of the private endpoint.')
param privateEndpointName string = '${appServiceName}-pe'

@description('Required. Resource tags.')
param tags object

// ============ //
// Variables    //
// ============ //

// Private DNS Zone for App Service
var privateDnsZoneName = 'privatelink.azurewebsites.net'

// ============ //
// Resources    //
// ============ //

// Deploy Private Endpoint for App Service
// MSLearn: https://learn.microsoft.com/azure/templates/microsoft.network/privateendpoints
resource privateEndpoint 'Microsoft.Network/privateEndpoints@2023-11-01' = {
  name: privateEndpointName
  location: location
  tags: tags
  properties: {
    subnet: {
      id: subnetResourceId
    }
    privateLinkServiceConnections: [
      {
        name: privateEndpointName
        properties: {
          privateLinkServiceId: appServiceResourceId
          groupIds: [
            'sites'
          ]
        }
      }
    ]
  }
}

// Deploy Private DNS Zone Group for automatic DNS registration
resource privateDnsZoneGroup 'Microsoft.Network/privateEndpoints/privateDnsZoneGroups@2023-11-01' = {
  parent: privateEndpoint
  name: 'default'
  properties: {
    privateDnsZoneConfigs: [
      {
        name: replace(privateDnsZoneName, '.', '-')
        properties: {
          privateDnsZoneId: resourceId('Microsoft.Network/privateDnsZones', privateDnsZoneName)
        }
      }
    ]
  }
}

// ============ //
// Outputs      //
// ============ //

@description('The resource ID of the private endpoint.')
output resourceId string = privateEndpoint.id

@description('The name of the private endpoint.')
output name string = privateEndpoint.name
